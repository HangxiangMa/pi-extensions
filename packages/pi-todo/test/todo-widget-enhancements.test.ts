...
test("adapts widget rows, prioritizes active work, and honors display settings", async () => {
  ...
  assert.deepEqual(compact, ["─".repeat(40), "Todo · 1/4 complete", "▶ current", "✓ 1 completed · … 2 more"]);
  ...
  assert.deepEqual(cappedAdaptive, ["─".repeat(40), "Todo · 1/4 complete", "▶ current", "✓ 1 completed · … 2 more"]);
...
test("bounds wrapped prioritized todos and sanitizes collapsed raw state without mutation", () => {
  ...
  assert.deepEqual(
    ...
    ["─".repeat(80), "Todo · 0/1 complete", "⚠ (text hidden after sanitization) — (reason hidden after sanitization)"],
  );
});
...
test("returns actionable validation errors before schema validation and direct execution", async () => {
  const cases: [unknown, RegExp][] = [
    ...
    [{ todos: Array.from({ length: 51 }, () => ({ step: "x", status: "pending" })) }, /maximum is 50/iu],
...
test("renders blocked todos, preserves reasons, and excludes them from completion", async () => {
  ...
  try {
    ...
    assert.deepEqual(lines, ["─".repeat(80), "Todo · 0/2 complete", "⚠ deploy — Need approval", "○ verify"]);
